...
void ChessBoard::update_Board(string org, string dest){
...
bool ChessBoard::is_in_check(piece_colour attacker,piece_colour attacked){
  ...
  for(it = map_cb.begin(); it != map_cb.end(); it++){
	...
	//cout<<"is valid move,inside is in check"<<endl;
	...
	if(!piece_in_the_way(it->first,King_attacked)){
	  ...
	  //cout<<"King attacked is in check"<<endl;
	    check_state = true;
	    return true;
	  }
      }
...
bool ChessBoard::can_move(piece_colour opponent){
...
bool ChessBoard::is_dest_occ_byOwn(string dest, piece_colour Own){
  map <string,Piece*>::iterator it;
  it = map_cb.find(dest);
...
bool ChessBoard::move_results_in_check(string org,string dest, piece_colour attacked){
...
bool ChessBoard::submitMove(string org, string dest){
 

  //check if org and dest are the same
  if(dest == org){
    return false;
    //cout<<"Cannot move to same place"<<endl;
  }  
  ...
  if(it->second->get_colour() != turn){
    cerr<<"It is not "<<output_colour(it->second->get_colour());
...
void ChessBoard::resetBoard(){
  ...
  for(char c = 'A';c < 'I'; c++){
    /*
    ASK FIDELIS WHY THIS DOESNT WORK
    char pos[White];
    char letter = c;
    char* l_ptr  = &letter;
    cout<<"deref pointer: "<<*l_ptr<<endl;
    strncpy(pos,l_ptr,White);    cout<<"after strncpy: "<<pos<<endl;
    strcat(pos,"2");*/
...
bool ChessBoard::piece_in_the_way(string org, string dest){
...
void ChessBoard::print_move(string org,string dest,map <string,Piece*>::iterator it){
  cout<<output_colour(turn)<<"'s "<<it->second->get_name();
  cout<<" moves from "<<org<<" to "<<dest;
}

void ChessBoard::print_invalid_move(string dest,map <string,Piece*>::iterator it){
...
void ChessBoard::change_position(string org,string dest){
  map<string,Piece*>::iterator it;
  it = map_cb.find(org);
  if(it == map_cb.end()){
